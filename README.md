...
![Halma GUI screenshot](assets/halma-gui.png)
...
```bash
python -m halma --size 8 --ai-color red --search-depth 3 --time-limit 20
```

## Headless arena (AI vs AI)

```bash
python -m halma.arena --games 5 --start-player alternate --depth-g 3 --depth-r 4 --time-limit 10
```

## Project layout
- `halma/ai.py` — minimax/alpha-beta logic and evaluation
- `halma/board.py` — rules, state, and move validation
- `halma/gui.py` — Tkinter interface
- `halma/arena.py` — headless benchmark runner
- `halma/__main__.py` — CLI entry point for the GUI
- `halma/tuning_notes.md` — example benchmark commands
